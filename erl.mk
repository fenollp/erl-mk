...
escript: | all
	@erl -noshell \
	     ...
	     -eval 'G = fun (_, [], Acc) -> Acc; (F, [Path|Rest], Acc) -> case filelib:is_dir(Path) of false -> F(F, Rest, [Path|Acc]); true -> {ok, Listing} = file:list_dir(Path), Paths = [filename:join(Path, Name) || Name <- Listing, Name /= ".git"], F(F, Paths ++ Rest, Acc) end end, escript:create("$(APP)", [ {shebang,default}, {comment,""}, {emu_args,"-pa $(APP)/ebin $(patsubst %,-pa $(APP)/%,$(wildcard deps/*/ebin))"}, {archive, [{"$(APP)/" ++ File -- "./",element(2,file:read_file(File))} || File <- G(G, ["."], [])], []}]).' \
