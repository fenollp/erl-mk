...
run (Function, Data) ->
    ...
    Kws = [begin
               ...
               #kwork{queue=Queue, local=Local, freq=Freq, units=K,
                      isize=ItemSize}
           end || Device <- E#cl.devices],
    io:format("Kws = ~p\n", [Kws]),

    %% Sum the weights and scale to [0..1]
    Tw = foldl(fun (K, Sum) ->
                       Sum + K#kwork.freq * K#kwork.units
               end, 0, Kws),
    Kws1 = [K#kwork{weight = K#kwork.freq * K#kwork.units / Tw} || K <- Kws],
    io:format("Kws1 = ~p\n", [Kws1]),
...
%%
%% Translates to
%%     __kernel main (__global T0        *input,
%%                    __global T0        *output,
...
p_fun ([{'fun',_Ln1,{clauses,[{clause,_Ln3,H,[],B}]}}]) ->
    ...
    {NArgs, ItemSize, lists:flatten([g_header(As), g_body(As,B)])};
p_fun (Fs) ->
    io:format("Fs=~p\n", [Fs]),
    erlang:error(not_supported).

p_header (Params) ->
    [p_arg(Param) || Param <- Params].
...
g_header ([{V,T}|Ps]) ->
    ["__kernel void example("
     ...
     "const uint n",
     [ [",const ", g_type(Tx), " ", atom_to_list(X)] || {X,Tx} <- Ps],
     ")\n"
     "{"
     "  size_t i = get_global_id(0);\n"
     "  if (i < n) {\n"
