...
INLINE static
lhash_bucket_t **
lhash_LOOKUP(lhash_t* lh, void* key)
{
...
lhash_t *
lhash_init(lhash_t* lh, char* name, int thres, lhash_func_t* func)
{
    lhash_bucket_t*** bp;
    ...
    if (lh->nactive >= lh->nslots) {
        ...
        if (LHASH_POS(lh->nactive) == 0) {
            ...
            if (six == lh->nsegs) {
                ...
                lh->seg = (lhash_bucket_t***)
                    realloc(lh->seg, sizeof(lhash_bucket_t**)*sz);
                lh->nsegs = sz;
                lh->n_resize++;
                for (i = six+1; i < sz; i++)
                    lh->seg[i] = 0;
...
lhash_t*
lhash_new(char* name, int thres, lhash_func_t* func)
{
    lhash_t* tp;
...
void*
lhash_insert_new(lhash_t* lh, void* key, void* data)
{
...
void*
lhash_Insert(lhash_t* lh, void* key, void* data)
{
    lhash_value_t hval = lh->func.hash(key);
    lhash_bucket_t** bpp = lhash_HLOOKUP(lh, hval, key);
    lhash_bucket_t* b = *bpp;
...
*/
void*
lhash_erase(lhash_t* lh, void* key)
{
    lhash_bucket_t** bpp = lhash_LOOKUP(lh, key);
    lhash_bucket_t* b = *bpp;
...
void
lhash_each(lhash_t* lh, void (elem)(lhash_t* lh, void* elem, void* arg),
           void* arg)
{
    int i;
